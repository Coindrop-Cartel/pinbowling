...
export function renderBaseballScoreboard(calcResult, machines, scoreMap, context, domRefs, engine) {
  ...
  if (eventMatchups.length === 0) {
    // Fall back to standard ScoringEngine rendering if no matchups exist
    ScoringEngine.prototype.renderResults.call(engine, calcResult, machines, scoreMap, context, domRefs);
    return;
  }
  ...
  playerResults.forEach(pResult => {
    ...
    for (let i = 0; i < pTurnResults.length; i++) {
      ...
      if (turn.played) {
        ...
        if (turn.isBattery && inningData[inningKey][playerIdNum] === undefined) {
