...
test.describe('Scoring Workflow', () => {
  test('should save a score and make field readonly', async ({ page }, testInfo) => {
    ...
    // 1. Setup Phase: Log in as admin to configure the league, event, and roster
    await page.goto('');
    ...
    // Expand League Row
    const leagueRow = page.locator('.league-registry-item', { hasText: leagueName }).first();
    await leagueRow.click();

    // Add Target Player to the League Roster
    await leagueRow.locator('button.add-player-btn').click();
    await page.selectOption('#player-select-modal', { label: targetPlayerName });
    ...
    await page.selectOption('#machine-select', { index: 1 });
    await page.fill('#value-10', '10000000');
    await page.fill('#value-1', '1000000');
    await page.click('#save-round-btn');
    await expect(page.locator('#rounds-list')).not.toBeEmpty();
    
    // Click DONE to complete setup and go back
    await page.click('#done-setup-btn');
    await expect(page.locator('#leagues-list')).toBeVisible();

    // 2. Play Phase: Log out and log in as the target testing role if not admin
    if (!testInfo.project.name.includes('admin')) {
      await ensureLoggedOut(page);
      const role = isPlayerProj ? 'player1' : 'td';
    ...
    // 3. Teardown Phase: Log back in as admin to clean up the created league
    await loginAs(page, 'admin', 'admin');
    ...
    const leagueRowToDelete = page.locator('.league-registry-item', { hasText: leagueName }).first();
    ...
    await expect(page.locator('#leagues-list')).not.toContainText(leagueName);
  });
});