...
describe('Teams Page (teamsPage.js)', () => {
  ...
  describe('Initialization', () => {
    it('should render teams list on initialization', async () => {
      ...
      PB_API.teams.getAll.mockResolvedValue(mockTeams);
      PB_API.players.getAll.mockResolvedValue([]);
      await initTeamsPage();
      expect(PB_API.teams.getAll).toHaveBeenCalled();
    ...
    it('should show empty message when no teams exist', async () => {
      PB_API.teams.getAll.mockResolvedValue([]);
      PB_API.players.getAll.mockResolvedValue([]);
      await initTeamsPage();
    ...
    it('should handle API errors gracefully during initialization', async () => {
      ...
      PB_API.teams.getAll.mockRejectedValue(new Error('Network Error'));
      await initTeamsPage();
    ...
    it('should hide management actions if not authorized', async () => {
      ...
      PB_API.teams.getAll.mockResolvedValue([{ id: 1, name: 'ReadOnly Team', members: [] }]);
      await initTeamsPage();
  ...
  describe('Edit Team', () => {
    ...
    it('should call updateTeam on form submit when editing', async () => {
      ...
      PB_API.teams.getAll.mockResolvedValue(mockTeams);
      PB_API.players.getAll.mockResolvedValue([]);
      PB_API.teams.update.mockResolvedValue({ success: true });
      await initTeamsPage();
      document.querySelector('.edit-team-btn').click();
      document.getElementById('team-name').value = 'Updated Team';
      document.getElementById('team-form').dispatchEvent(new Event('submit'));
      expect(PB_API.teams.update).toHaveBeenCalledWith(123, expect.objectContaining({
        name: 'Updated Team',
      }));
    });

    it('should not submit if team name is empty', async () => {
      PB_API.teams.getAll.mockResolvedValue([]); // Override default for this test
      PB_API.players.getAll.mockResolvedValue([]);
      await initTeamsPage();
      document.getElementById('team-name').value = '';
      document.getElementById('team-form').dispatchEvent(new Event('submit'));
      expect(PB_API.teams.create).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Create Team', () => {
    it('should call PB_API.createTeam for new team submission', async () => {
      PB_API.teams.getAll.mockResolvedValue([]);
      PB_API.players.getAll.mockResolvedValue([]);
      PB_API.teams.create.mockResolvedValue({ success: true });
      await initTeamsPage();
      ...
      document.getElementById('team-form').dispatchEvent(new Event('submit'));
      expect(PB_API.teams.create).toHaveBeenCalledWith(expect.objectContaining({
        name: 'New Team',
        city: 'Houston',
        state: 'TX',
      }));
    ...
    it('should handle API error on create', async () => {
      PB_API.teams.getAll.mockResolvedValue([]);
      PB_API.players.getAll.mockResolvedValue([]);
      PB_API.teams.create.mockRejectedValue(new Error('Create failed'));
      await initTeamsPage();
      document.getElementById('team-name').value = 'New Team';
      document.getElementById('team-form').dispatchEvent(new Event('submit'));
      await vi.waitFor(() => {
  ...
  describe('Delete Team', () => {
    it('should delete a team after confirmation', async () => {
      ...
      PB_API.teams.delete.mockResolvedValue({ success: true });
      await initTeamsPage();
      ...
      await vi.waitFor(() => {
        ...
        expect(PB_API.teams.delete).toHaveBeenCalledWith(55);
        expect(PB_API.teams.getAll).toHaveBeenCalledTimes(2);
      });
    });
    ...
    it('should not delete if user cancels confirmation', async () => {
      ...
      deleteBtn.click();
      await vi.waitFor(() => {
        expect(PB_API.teams.delete).not.toHaveBeenCalled();
      });
    });
    ...
    it('should handle API error on delete', async () => {
      ...
      PB_API.players.getAll.mockResolvedValue([]);
      showConfirm.mockResolvedValue(true);
      PB_API.teams.delete.mockRejectedValue(new Error('Delete failed'));
      await initTeamsPage();
      const deleteBtn = document.querySelector('.delete-team-btn');
      deleteBtn.click();
      await vi.waitFor(() => {
  ...
  describe('Add Member', () => {
    it('should add a member when add button is clicked', async () => {
      ...
      PB_API.teams.getAll.mockResolvedValue(mockTeams);
      PB_API.players.getAll.mockResolvedValue([{ id: 77, playerName: 'New Player' }]);
      showPlayerSelectionDialog.mockResolvedValue('77');
      PB_API.teams.addMember.mockResolvedValue({ success: true });
      await initTeamsPage();
      ...
      await vi.waitFor(() => {
        ...
        expect(PB_API.teams.addMember).toHaveBeenCalledWith(1, "77");
      });
    });
    ...
    it('should show alert when all players are already assigned', async () => {
      const mockTeams = [{ id: 1, name: 'Team 1', members: [{ id: 99, playerName: 'John Doe' }] }]; // Override default for this test
      PB_API.teams.getAll.mockResolvedValue(mockTeams);
      PB_API.players.getAll.mockResolvedValue([{ id: 99, playerName: 'John Doe' }]);
      await initTeamsPage();
      document.querySelector('.row-header').click();
      const addMemberBtn = document.querySelector('.add-member-btn');
      addMemberBtn.click();
    ...
    it('should not add member if dialog is cancelled', async () => {
      ...
      addMemberBtn.click();
      await vi.waitFor(() => {
        expect(PB_API.teams.addMember).not.toHaveBeenCalled();
      });
    });
    ...
    it('should handle API error on add member', async () => {
      ...
      PB_API.players.getAll.mockResolvedValue([{ id: 77, playerName: 'New Player' }]);
      showPlayerSelectionDialog.mockResolvedValue('77');
      PB_API.teams.addMember.mockRejectedValue(new Error('Add member failed'));
      await initTeamsPage();
      document.querySelector('.row-header').click();
      const addMemberBtn = document.querySelector('.add-member-btn');
      addMemberBtn.click();
  ...
  describe('Remove Member', () => {
    it('should remove a member when remove button is clicked', async () => {
      ...
      PB_API.players.getAll.mockResolvedValue([]);
      showConfirm.mockResolvedValue(true);
      PB_API.teams.removeMember.mockResolvedValue({ success: true });
      await initTeamsPage();
      document.querySelector('.row-header').click();
      ...
      await vi.waitFor(() => {
        ...
        expect(PB_API.teams.removeMember).toHaveBeenCalledWith("1", "99");
      });
    });
    ...
    it('should not remove member if user cancels confirmation', async () => {
      ...
      removeBtn.click();
      await vi.waitFor(() => {
        expect(PB_API.teams.removeMember).not.toHaveBeenCalled();
      });
    });
    ...
    it('should handle API error on remove member', async () => {
      ...
      PB_API.players.getAll.mockResolvedValue([]);
      showConfirm.mockResolvedValue(true);
      PB_API.teams.removeMember.mockRejectedValue(new Error('Remove failed'));
      await initTeamsPage();
      document.querySelector('.row-header').click();
      const removeBtn = document.querySelector('.remove-member-btn');
      removeBtn.click();
  ...
  describe('API Error Handling on Save', () => {
    it('should handle API error on update team', async () => {
      ...
      PB_API.teams.getAll.mockResolvedValue(mockTeams);
      PB_API.players.getAll.mockResolvedValue([]);
      PB_API.teams.update.mockRejectedValue(new Error('Update failed'));
      await initTeamsPage();
      document.querySelector('.edit-team-btn').click();
      document.getElementById('team-name').value = 'Updated Team';
      document.getElementById('team-form').dispatchEvent(new Event('submit'));
